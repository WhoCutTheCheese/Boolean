...
const command: BooleanCommand = {
	...
	callback: async (client: Client, message: Message, args: string[]) => {
		...
		user.setNickname(after).catch((err: Error) => {
			console.error(err);
			message.channel.send({ content: "An unknown error occurred! If this issue persists, please join our support server.", });
		});

		const sanitized = new EmbedBuilder().setDescription(`You have sanitized \`${user.user.tag}\`'s name.\n**New Nickname:** ${after}`).setColor(color);
