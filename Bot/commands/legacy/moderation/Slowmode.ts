...
const command: BooleanCommand = {
	...
	callback: async (client: Client, message: Message, args: string[]) => {
		...
		if (!getLenthFromString) return new EmbedUtils().sendEmbed(EmbedType.error, (message.channel as TextChannel), { message: message, emoji: true, replyToMessage: true, deleteMsg: true }, { description: `Invalid time provided` });
		let [length, lengthString] = getLenthFromString

		if (length < 0) return new EmbedUtils().sendEmbed(EmbedType.error, (message.channel as TextChannel), { message: message, emoji: true, replyToMessage: true, deleteMsg: true }, { description: `Slowmode must be at least 1 second!` });
		if (length > 21600) return new EmbedUtils().sendEmbed(EmbedType.error, (message.channel as TextChannel), { message: message, emoji: true, replyToMessage: true, deleteMsg: true }, { description: `Slowmode must be under 6 hours!` });
		...
		try {
			...
			new EmbedUtils().sendEmbed(EmbedType.success, (message.channel as TextChannel), { message: message, emoji: true, replyToMessage: true }, { title: "Set slowmode", description: `Slowmode set to **${lengthString}**` });
