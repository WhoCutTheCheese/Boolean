...
module.exports = {
  ...
  callback: async (client: Client, message: Message, args: string[]) => {
    ...
    let color: ColorResolvable = await new Utilities().getEmbedColor(
      message.guild!
    );
    ...
    if (!user)
      return message.channel.send({
        content: "Unable to fetch that member! Please try again.",
      });
    ...
    user.setNickname(newNick).catch((err: Error) => {
      ...
      message.channel.send({
        content:
          "An unknwon error occurred! If this issue persists, please join our support server.",
      });
    });
    ...
    new EmbedUtils().sendModLogs(
      ...
      { title: action, actionInfo: `\`${oldNick}\` -> \`${newNick}\`` }
    );
  },
};
